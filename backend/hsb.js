...
async function getDatazLoggedinPage(page) {
    ...
    await page.goto('https://secure.handelsbanken.se/se/private/sv/#!/accounts_and_cards')
    await page.waitForSelector(('[data-spif-id="shb-priv-accounts-and-cards"] tr.shb-js-grid-row'))

    let $table2 = await page.evaluate(() => {
        let elements = document.querySelectorAll('[data-spif-id="shb-priv-accounts-and-cards"] tr.shb-js-grid-row');
...
async function getInfDataz() {
    try {
        ...
        setInterval(async () => {
            ...
            const diffs = values.map((newVal, index) => ({ index, val: newVal }))
                .filter(obj => lastValues[obj.index] !== obj.val);
            diffs.map(({ index, val }) => {
                console.log(`${keys[index - 1]}\t Nytt: ${val}, innan: ${lastValues[index]}`);
            })
            lastValues = values;
